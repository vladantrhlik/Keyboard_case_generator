...
e = 0.01;
module keyboard(file, width, height, depth, thickness,
           ...
           padding)
{
  ...
  if (assemble){
      difference(){
      box3d();
      translate([padding+1, padding+1, height-thickness]) cube(size = [width-padding*2-2,depth-padding*2-2,thickness]);  
      }
      translate([padding, padding, height-thickness]){
      ...
      group(){
            difference(){
          box2d();  
          translate([ padding+1, height+padding+1]) square(size=[width-2*padding-2,      depth-2*padding-2]); 
          
      }
      translate([ padding, height+padding]) import(file);
      }
